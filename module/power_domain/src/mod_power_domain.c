...
static int get_highest_level_from_composite_state(const struct pd_ctx *pd,
    uint32_t composite_state)
{
    uint32_t state;
...
static int complete_system_suspend(struct pd_ctx *target_pd)
{
    ...
    do {
        ...
        composite_state |= ((pd->parent != NULL) ? MOD_PD_STATE_OFF :
                            mod_pd_ctx.system_suspend.state) << shift;
        pd = pd->parent;
        level++;
                                ...
                                const void *config)
{
    ...
    if ((pd_config->composite_state_mask_table != NULL) &&
        ...
        pd->composite_state_mask_table =
            pd_config->composite_state_mask_table;
        ...
        pd->cs_support = true;
            ...
            pd->composite_state_levels_mask =
                MOD_PD_CS_STATE_MASK << MOD_PD_CS_LEVEL_SHIFT;
            pd->cs_support = true;
